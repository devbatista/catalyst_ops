...
RSpec.describe Client, type: :model do
  ...
  describe 'document methods' do
    let(:cpf_client) { create(:client, document: CPF.generate) }
    let(:cnpj_client) { create(:client, document: CNPJ.generate) }
    ...
    it 'formats document correctly' do
      expect(cpf_client.formatted_document).to eq('111.444.777-35')
      expect(cnpj_client.formatted_document).to eq('11.222.333/0001-81')
    end
  end
  ...
  describe 'phone formatting' do
    ...
    it 'returns original phone for invalid format' do
      client = create(:client, phone: '123')
      expect(client.formatted_phone).to eq('123')
    end
  end
