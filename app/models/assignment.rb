class Assignment < ApplicationRecord
  ...
  def user_availability
    ...
    conflicting_assignments = user.assignments
                                 .joins(:order_service)
                                 .where.not(order_services: { status: [:concluida, :cancelada] })
                                 .where.not(id: id)
                                 .where(order_services: { scheduled_at: order_service.scheduled_at.beginning_of_day..order_service.scheduled_at.end_of_day })
