# Documentação Para Agentes De IA

Esta pasta concentra contexto de desenvolvimento para agentes de IA. Ela não
substitui `README.md`, `README_TECNICO.md` nem a central de ajuda em `docs/`;
ela aponta quais regras de negócio e riscos devem ser considerados antes de
alterar código.

## Como Usar

- Comece por `AGENTS.md` na raiz do projeto.
- Leia o arquivo de domínio relacionado à tarefa.
- Quando uma tarefa cruzar domínios, leia todos os arquivos envolvidos.
- Para fluxos de usuário, consulte também `docs/gestor/` e `docs/tecnico/`.

## Domínios

- [Assinaturas e cobrança](assinaturas.md)
- [Empresa e tenancy](empresas_e_tenancy.md)
- [Usuários e permissões](usuarios_e_permissoes.md)
- [Ordens de serviço](ordens_de_servico.md)
- [Orçamentos](orcamentos.md)
- [Clientes](clientes.md)
- [Agenda e atribuições](agenda_e_atribuicoes.md)
- [Suporte e base de conhecimento](suporte_e_base_conhecimento.md)
- [Financeiro e relatórios](financeiro_e_relatorios.md)
- [Integrações e webhooks](integracoes_e_webhooks.md)
- [Auditoria e operação](auditoria_e_operacao.md)
- [API mobile](api_mobile.md)
- [Admin e backoffice](admin_backoffice.md)
- [Onboarding de usuários](onboarding.md)
- [Portal público do cliente](portal_cliente.md)
